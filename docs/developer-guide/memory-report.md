...
For Phase 49 trig, reports show `__rt_f32_sin` / `__rt_f32_cos` wrappers when used, shared `__rt_f32_sincos_core`, and the selected internal ROM table for sine/cosine. Dynamic `tanf` reports `__rt_f32_tan -> __rt_f32_tan_core`; tan-only programs no longer emit `__rt_f32_sincos_core`. `compact` reports `variant=shared_core_compact` or `variant=tan_core_compact`; default `balanced` reports `variant=shared_core_balanced` or `variant=tan_core_balanced`. The accuracy string records scoped moderate aliases through `±8pi`, non-pole tangent tolerance, and finite tangent pole saturation. Dynamic precise-profile trig is deferred and fails before a final report is emitted.
