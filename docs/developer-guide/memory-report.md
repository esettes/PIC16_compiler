...
For Phase 50 trig, reports include `Trig runtime strategy`. Sin/cos-only reports `sincos_shared_core`; tan-only reports `isolated_tan_core`; combined sin/cos/tan reports `combined_sincos_tan_core`. Dynamic tan-only reports `__rt_f32_tan -> __rt_f32_tan_core`; combined dynamic trig reports `__rt_f32_tan -> __rt_f32_sincos_core`. `compact` reports compact variants; default `balanced` reports balanced variants. The accuracy string records scoped moderate aliases through `±8pi`, non-pole tangent tolerance, and finite tangent pole saturation. Dynamic precise-profile trig is deferred and fails before a final report is emitted.
