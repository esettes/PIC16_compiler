...
Inputs are radians. Dynamic compact and balanced `sinf` / `cosf` helpers use one shared `__rt_f32_sincos_core`, internal ROM quarter-wave tables, and validated table points in `[-2π, +2π]`. Compact tolerance is `<= 0.10` for `sinf` / `cosf`; balanced tolerance is `<= 0.05` for simulator-validated `sinf` / `cosf` points. Phase 49 `tanf` uses an isolated `__rt_f32_tan_core` with non-pole tolerance `<= 0.20` for compact and `<= 0.10` for balanced; tan-only code does not emit `__rt_f32_sincos_core`. Pole-like odd `π/2` inputs return finite saturation `+32767.0f` or `-32767.0f`; no Inf is produced. Phase 46 validates deterministic common-multiple aliases through `±8π`. Phase 47 compacts sine/cosine aliases by matching positive magnitudes once and applying sine signs afterward. Outside documented points, runtime fallback is coarse and finite; use ROM calibration tables or fixed-point if exact trig behavior matters.
