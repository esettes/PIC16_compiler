...
Phase 38/39 math-profile validation lives in the same suites. It checks CLI parsing for `--math-profile`, compact/balanced `sqrtf` behavior, precise `sqrtf` helper emission, compact-vs-precise size and accuracy comparisons, checked-in precise examples, and simulator execution for refined non-perfect roots.
...
Phase 45 trig validation checks finite `sinf` / `cosf` semantic diagnostics, shared-core pruning, internal ROM math table visibility, compact/balanced math-profile reporting, ISR rejection, checked-in examples, size reduction, simulator accuracy grids for compact/balanced profiles, and common `±3pi` / `±4pi` aliases.
