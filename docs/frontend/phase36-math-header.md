...
- `sinf` / `cosf` / `tanf` are Phase 43/48 finite table-backed approximations in radians
- `fabsf` may appear in an ISR because backend lowers it inline
- `truncf`, `floorf`, `ceilf`, `roundf`, `sqrtf`, `fminf`, `fmaxf`, `sinf`, `cosf`, and `tanf` are rejected in ISRs because they require runtime helpers
